...
import type { IAppConfig } from '../../shared/constants/apps-config/interfaces/app-list.interface'
import type { IPlatformGuideProps } from './interfaces/platform-guide.props.interface'
    ...
    <Box>
          ...
          {platformApps.map((app: IAppConfig) => {
            ...
            return (
              <Button
                ...
                key={app.id}
                leftSection={app.isFeatured ? <IconStar color="gold" size={16} /> : undefined}
      ...
      <Timeline active={1} bulletSize={32} color="teal" lineWidth={2}>
        ...
        {selectedApp?.additionalBeforeAddSubscriptionStep && (
          ...
          >
            <Text c="dimmed" mb={16} size="sm">
              {selectedApp.additionalBeforeAddSubscriptionStep.description[currentLang] ||
                selectedApp.additionalBeforeAddSubscriptionStep.description.en}
            </Text>
            <Group>
              {selectedApp.additionalBeforeAddSubscriptionStep.buttons.map((button, index) => (
                <Button
                  component="a"
        ...
        >
          ...
          {selectedApp && (
            <Button
              onClick={() => openDeepLink(selectedApp.urlScheme, selectedApp.isNeedBase64Encoding)}
              variant="filled"
            >
        ...
        {selectedApp?.additionalAfterAddSubscriptionStep && (
          ...
          >
            <Text c="dimmed" mb={16} size="sm">
              {selectedApp.additionalAfterAddSubscriptionStep.description[currentLang] ||
                selectedApp.additionalAfterAddSubscriptionStep.description.en}
            </Text>
            <Group>
              {selectedApp.additionalAfterAddSubscriptionStep.buttons.map((button, index) => (
                <Button
                  component="a"
                  href={button.buttonLink}
                  key={button.buttonLink}
